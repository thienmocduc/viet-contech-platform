...
export function createDashboardRouter() {
    ...
    app.use('*', requireAuth);
    app.get('/overview', (c) => {
        const projectsByStatus = query(`SELECT status, COUNT(*) AS c FROM projects GROUP BY status ORDER BY status`);
        const runs24h = queryOne(`SELECT COUNT(*) AS c FROM agent_runs WHERE started_at >= datetime('now','-1 day')`);
        const qcPass = queryOne(`SELECT SUM(CASE WHEN status IN ('passed','auto_fixed') THEN 1 ELSE 0 END) AS pass,
              COUNT(*) AS total
       FROM qc_gates`);
        const avgDur = queryOne(`SELECT AVG(duration_ms) AS avg_ms FROM agent_runs WHERE duration_ms IS NOT NULL`);
        const totalRevenue = queryOne(`SELECT SUM(total_vnd) AS total FROM boq_items`);
        return c.json({
            ok: true,
    ...
    app.get('/agent-stats', (c) => {
        const rows = query(`SELECT agent_id,
              COUNT(*) AS total,
    ...
    app.get('/recent-activity', (c) => {
        const rows = query(`SELECT id, project_id, action, actor, target_type, target_id, ts
