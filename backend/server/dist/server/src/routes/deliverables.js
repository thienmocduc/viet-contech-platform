...
function ensureMine(projectId, userId) {
    return queryOne('SELECT id, code, name, owner_user_id FROM projects WHERE id=? AND owner_user_id=?', [projectId, userId]);
}
const KIND_TO_MIME = {
    dxf: 'application/dxf',
...
export function createDeliverableRouter() {
    const app = new Hono();
    app.use('*', requireAuth);
    // GET /:projectId
    app.get('/:projectId', (c) => {
        ...
        if (!ensureMine(projectId, user.id))
            return c.json({ ok: false, error: 'not_found' }, 404);
        const items = query(`SELECT * FROM deliverables WHERE project_id=? ORDER BY created_at DESC LIMIT 1000`, [projectId]);
    ...
    app.get('/:projectId/:revId/:code', async (c) => {
        ...
        if (!ensureMine(projectId, user.id))
            return c.json({ ok: false, error: 'not_found' }, 404);
        const item = queryOne(`SELECT * FROM deliverables
       WHERE project_id=? AND revision_id=?
         AND (path LIKE ? OR path LIKE ?)
       ORDER BY version DESC LIMIT 1`, [projectId, revId, `%/${code}.%`, `%/${code}-%`]);
        if (!item)
            return c.json({ ok: false, error: 'deliverable_not_found' }, 404);
        audit({
    ...
    app.get('/:projectId/manifest', (c) => {
        ...
        if (!project)
            return c.json({ ok: false, error: 'not_found' }, 404);
        const items = query(`SELECT * FROM deliverables WHERE project_id=? ORDER BY created_at ASC`, [projectId]);
        const byKind = {};
        let totalSize = 0;
