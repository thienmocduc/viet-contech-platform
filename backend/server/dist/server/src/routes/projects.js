...
function loadProjectMine(id, userId) {
    return queryOne('SELECT * FROM projects WHERE id=? AND owner_user_id=?', [id, userId]);
}
// ============================================================
// Router
// ============================================================
export function createProjectRouter() {
    ...
    app.post('/', async (c) => {
        ...
        const now = nowIso();
        tx(() => {
            exec(`INSERT INTO projects (id, code, name, owner_user_id, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, 'draft', ?, ?)`, [id, code, parsed.data.name, user.id, now, now]);
            exec(`INSERT INTO project_revisions (id, project_id, parent_revision_id, message, agent, created_at)
         VALUES (?, ?, NULL, ?, NULL, ?)`, [revId, id, 'rev:0 — initial brief', now]);
            // Lot spec
            if (parsed.data.lot) {
                const lot = parsed.data.lot;
                const area = lot.width_m * lot.depth_m;
                exec(`INSERT INTO lot_specs (project_id, width_m, depth_m, area_m2, direction, address, gfa_target)
           VALUES (?, ?, ?, ?, ?, ?, ?)`, [id, lot.width_m, lot.depth_m, area, lot.direction, lot.address ?? null, lot.gfa_target ?? null]);
            }
            // Client profile
            if (parsed.data.client) {
                const cp = parsed.data.client;
                exec(`INSERT INTO client_profile (project_id, full_name, phone, year_born, gender, family_size)
           VALUES (?, ?, ?, ?, ?, ?)`, [
                    id,
                    cp.full_name,
                    cp.phone ?? null,
            ...
            for (const r of parsed.data.requirements) {
                exec(`INSERT INTO requirements (id, project_id, source, type, key, value, locked)
           VALUES (?, ?, 'brief', ?, ?, ?, 0)`, [uid('req'), id, r.type, r.key, r.value]);
            }
            return null;
        });
        audit({
    ...
    app.get('/', (c) => {
        const user = c.get('user');
        const items = query(`SELECT * FROM projects WHERE owner_user_id=? ORDER BY created_at DESC LIMIT 200`, [user.id]);
    ...
    app.get('/:id', (c) => {
        ...
        const revisions = query('SELECT * FROM project_revisions WHERE project_id=? ORDER BY created_at ASC', [id]);
        const runStats = query(`SELECT status, COUNT(*) AS c FROM agent_runs WHERE project_id=? GROUP BY status`, [id]);
        const qcCount = queryOne(`SELECT COUNT(*) AS c FROM qc_gates WHERE project_id=? AND status IN ('passed','auto_fixed')`, [id]);
        return c.json({
            ok: true,
            project: p,
            revisions,
    ...
    app.patch('/:id', async (c) => {
        ...
        exec(`UPDATE projects SET name=?, status=?, updated_at=? WHERE id=?`, [next.name, next.status, next.updated_at, id]);
        audit({
            project_id: id,
    ...
    app.post('/:id/brief/analyze', async (c) => {
        ...
        const lot = queryOne('SELECT * FROM lot_specs WHERE project_id=?', [
            id,
        ]);
        const client = queryOne('SELECT * FROM client_profile WHERE project_id=?', [id]);
        const reqs = query('SELECT type, key, value FROM requirements WHERE project_id=?', [id]);
        const brief = {
            project_id: id,
            lot,
            client,
        ...
        let runner = null;
        try {
            runner = await loadExternal('pipeline/src/agent-runner.js');
        }
        catch {
            runner = null;
        }
    ...
    app.post('/:id/start-pipeline', (c) => {
        ...
        tx(() => {
            exec(`INSERT INTO project_revisions (id, project_id, parent_revision_id, message, agent, created_at)
         VALUES (?, ?, NULL, 'rev — start pipeline', NULL, ?)`, [revId, id, now]);
            exec(`UPDATE requirements SET locked=1 WHERE project_id=?`, [id]);
            exec(`UPDATE projects SET status='running', locked_revision_id=?, updated_at=? WHERE id=?`, [revId, now, id]);
            return null;
        });
        audit({
            project_id: id,
    ...
    app.get('/:id/pipeline', (c) => {
        ...
        if (!p)
            return c.json({ ok: false, error: 'not_found' }, 404);
        const phases = query(`SELECT phase,
              COUNT(*) AS total,
       ...
       GROUP BY phase
       ORDER BY phase`, [id]);
        const conflicts = queryOne(`SELECT COUNT(*) AS c FROM conflicts WHERE project_id=? AND status='open'`, [id]);
        const qcGates = query(`SELECT gate_code, status FROM qc_gates WHERE project_id=? ORDER BY gate_code`, [id]);
        return c.json({
            ok: true,
            project_id: id,
            status: p.status,
    ...
    app.get('/:id/revisions', (c) => {
        const user = c.get('user');
        const id = c.req.param('id');
        if (!loadProjectMine(id, user.id))
            return c.json({ ok: false, error: 'not_found' }, 404);
        const rows = query('SELECT * FROM project_revisions WHERE project_id=? ORDER BY created_at ASC', [id]);
    ...
    app.post('/:id/revisions/:revId/restore', (c) => {
        ...
        if (!p)
            return c.json({ ok: false, error: 'not_found' }, 404);
        const rev = queryOne('SELECT * FROM project_revisions WHERE id=? AND project_id=?', [revId, id]);
        if (!rev)
            return c.json({ ok: false, error: 'revision_not_found' }, 404);
        const newRev = uid('rev');
        const now = nowIso();
        exec(`INSERT INTO project_revisions (id, project_id, parent_revision_id, message, agent, created_at)
       VALUES (?, ?, ?, ?, NULL, ?)`, [newRev, id, revId, `restore from ${revId}`, now]);
        exec(`UPDATE projects SET status='review', locked_revision_id=?, updated_at=? WHERE id=?`, [newRev, now, id]);
        audit({
            project_id: id,
