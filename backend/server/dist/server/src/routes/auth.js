...
async function issueSessionAndCookie(c, user, ip, ua) {
    const jti = crypto.randomUUID();
    const expiresAt = new Date(Date.now() + env.JWT_TTL_SECONDS * 1000).toISOString();
    exec(`INSERT INTO sessions (id, user_id, jti, ip, ua, expires_at)
     VALUES (?, ?, ?, ?, ?, ?)`, [uid('sess'), user.id, jti, ip, ua, expiresAt]);
...
export function createAuthRouter() {
    ...
    app.post('/register/verify', async (c) => {
        ...
        const v = verifyOtp(parsed.data.sessionId, parsed.data.otp);
        if (!v.ok || !v.payload) {
            return c.json({ ok: false, error: v.error ?? 'verify_failed' }, 401);
        }
        // Race: re-check email
        const dup = queryOne('SELECT id FROM users WHERE email=?', [v.payload.email]);
        if (dup)
            return c.json({ ok: false, error: 'email_exists' }, 409);
        const id = uid('usr');
        exec(`INSERT INTO users (id, email, name, year_born, phone_zalo, role, status, last_login_at)
       VALUES (?, ?, ?, ?, ?, ?, 'active', ?)`, [id, v.payload.email, v.payload.name, v.payload.year, v.payload.phone, v.payload.role, nowIso()]);
        audit({
        ...
        await issueSessionAndCookie(c, user, c.req.header('x-forwarded-for') ?? null, c.req.header('user-agent') ?? null);
    ...
    app.post('/login', async (c) => {
        ...
        if (!parsed.success)
            return c.json({ ok: false, error: 'invalid_input' }, 400);
        const u = queryOne('SELECT id, email, name, role, status FROM users WHERE email=?', [parsed.data.email]);
    ...
    app.post('/login/verify', async (c) => {
        ...
        const u = queryOne('SELECT id, email, name, role, status FROM users WHERE email=?', [v.payload.email]);
        if (!u || u.status !== 'active')
            return c.json({ ok: false, error: 'no_account' }, 404);
        exec('UPDATE users SET last_login_at=? WHERE id=?', [nowIso(), u.id]);
        await issueSessionAndCookie(c, u, c.req.header('x-forwarded-for') ?? null, c.req.header('user-agent') ?? null);
        audit({
            action: 'auth.login',
            actor: u.id,
            target_type: 'user',
