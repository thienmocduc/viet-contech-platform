...
export function agentsCount() {
    return getAgents().length;
}
let _runnerCache = null;
async function getRunner() {
    if (_runnerCache)
        return _runnerCache;
    try {
        _runnerCache = await loadExternal('pipeline/src/agent-runner.js');
        return _runnerCache;
    }
    catch (e) {
        // eslint-disable-next-line no-console
        console.warn('[agents] runAgent loader fail:', e.message);
        return null;
    }
}
// ============================================================
 ...
 */
function mapAgentPhaseToPhaseCode(phase) {
    if (!phase)
        return undefined;
    const p = phase.toLowerCase();
    if (p.startsWith('b0'))
        return 'B1-Brief';
    if (p.startsWith('b1') || p.includes('brief'))
        return 'B1-Brief';
    if (p.startsWith('b2') || p.includes('phongthuy') || p.includes('concept'))
        return 'B2-Concept';
    if (p.startsWith('b3') || p.includes('layout'))
        return 'B3-Layout';
    if (p.startsWith('b4') || p.includes('struct'))
        return 'B4-Structural';
