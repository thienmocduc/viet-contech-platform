...
export function createHealthRouter() {
    ...
    app.get('/healthz', (c) => {
        ...
        const recentRuns = (() => {
            try {
                const r = queryOne(`SELECT COUNT(*) AS c FROM agent_runs WHERE started_at >= datetime('now','-1 day')`);
                return r?.c ?? 0;
            }
            catch {
                return 0;
        ...
        const tcvnCount = (() => {
            try {
                const r = queryOne(`SELECT COUNT(*) AS c FROM tcvn_rules`);
                return r?.c ?? 0;
            }
            catch {
                return 0;
        ...
        const allOk = tbl.ready >= 18 && ag >= 1 && pl.ready === true; // 18 = Wave-1 schema; users/sessions are bonus
    ...
    app.get('/livez', (c) => c.json({ ok: true, alive: true, uptime_seconds: Math.round((Date.now() - STARTED_AT) / 1000) }));
    return app;
}
...
export function topActivity() {
    try {
        return query(`SELECT action, COUNT(*) AS c FROM audit_log
