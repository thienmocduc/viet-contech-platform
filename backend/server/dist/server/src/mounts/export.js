...
export function mountExportRoutes(app) {
    ...
    app.post('/api/export/zip', async (c) => {
        ...
        const proj = queryOne('SELECT id, code, owner_user_id FROM projects WHERE id=?', [parsed.data.project_id]);
        if (!proj || proj.owner_user_id !== user.id) {
            return c.json({ ok: false, error: 'not_found' }, 404);
        }
        const items = query(`SELECT id, path, kind, size_bytes FROM deliverables WHERE project_id=?`, [proj.id]);
    ...
    app.get('/api/export/health', (c) => c.json({ ok: true, service: 'packager', export_dir: EXPORT_DIR }));
}
