...
async function ensureOrchestrator() {
    ...
    loading = (async () => {
        try {
            const mod = await loadExternal('pipeline/src/orchestrator.js');
            orchestrator = new mod.PipelineOrchestrator();
            orchestrator.on('pipeline_event', (ev) => {
            ...
            console.warn('[pipeline] orchestrator load failed:', e.message);
        }
    })();
    return loading;
}
...
async function startJob(projectId, brief) {
...
export function mountPipelineRoutes(app) {
    ...
    app.post('/api/pipeline/run/:projectId', async (c) => {
        ...
        const proj = queryOne(`SELECT id FROM projects WHERE id=?`, [projectId]);
        if (!proj)
    ...
    app.post('/api/pipeline/phase', async (c) => {
        await ensureOrchestrator();
        if (!orchestrator)
            return c.json({ ok: false, error: 'orchestrator_unavailable' }, 503);
        const body = await c.req.json().catch(() => null);
    ...
    app.get('/api/pipeline/stream/:jobId', (c) => {
        ...
        return stream(c, async (writer) => {
            ...
            const hb = setInterval(() => {
                ...
                if (cur)
                    void send({ type: 'snapshot', data: cur });
            }, 5_000);
            writer.onAbort(() => {
                unsub();
                clearInterval(hb);
            });
            await new Promise((resolve) => writer.onAbort(() => resolve()));
        });
    });
}
// Test-only — used for e2e smoke
export function _hasJobsInMem() {
    return jobs.size;
}
